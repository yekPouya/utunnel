...
در اسکریپت `utunnel_managernl.sh`، پارامترهای مربوط به MUX (Multiplexing) برای بهینه‌سازی عملکرد تونل استفاده شده‌اند. این پارامترها در بخش‌های مختلفی از کانفیگ‌های سرور و کلاینت ظاهر می‌شوند. در ادامه، توضیح هر یک از این پارامترها و تأثیر تغییر مقادیر آنها ارائه می‌شود:

---

### ۱. **`mux_framesize = 32768`**
- **معنی**:  
  این پارامتر اندازه هر فریم (واحد انتقال داده) در ارتباط MUX را تعیین می‌کند. مقدار پیش‌فرض `32768` بایت (معادل ۳۲ کیلوبایت) است.

- **تأثیر تغییر مقدار**:
  - **افزایش مقدار** (مثلاً `65536`):  
    - **مزیت**: کاهش سربار (overhead) ناشی از تقسیم داده به فریم‌های کوچک و بهبود عملکرد برای انتقال داده‌های حجیم.  
    - **عیب**: افزایش مصرف حافظه و تأخیر در انتقال داده‌های کوچک (چون باید فریم پر شود).  
  - **کاهش مقدار** (مثلاً `16384`):  
    - **مزیت**: مناسب برای ارتباطات با داده‌های کوچک و کاهش تأخیر.  
    - **عیب**: افزایش سربار به دلیل تعداد بیشتر فریم‌ها.

---

### ۲. **`mux_recievebuffer = 4194304`**
- **معنی**:  
  این پارامتر اندازه بافر دریافت (Receive Buffer) برای داده‌های MUX را تعیین می‌کند. مقدار پیش‌فرض `4194304` بایت (معادل ۴ مگابایت) است.

- **تأثیر تغییر مقدار**:
  - **افزایش مقدار** (مثلاً `8388608`):  
    - **مزیت**: بهبود عملکرد در شبکه‌های پرترافیک یا با تاخیر بالا (مثلاً شبکه‌های ماهواره‌ای).  
    - **عیب**: مصرف حافظه بیشتر و احتمال افزایش تأخیر در پردازش داده‌ها.  
  - **کاهش مقدار** (مثلاً `2097152`):  
    - **مزیت**: کاهش مصرف حافظه و مناسب برای دستگاه‌های با منابع محدود.  
    - **عیب**: احتمال از دست رفتن داده در صورت پر شدن بافر در ترافیک سنگین.

---

### ۳. **`mux_streambuffer = 65536`**
- **معنی**:  
  این پارامتر اندازه بافر برای هر جریان (Stream) جداگانه در MUX را تعیین می‌کند. مقدار پیش‌فرض `65536` بایت (معادل ۶۴ کیلوبایت) است.

- **تأثیر تغییر مقدار**:
  - **افزایش مقدار** (مثلاً `131072`):  
    - **مزیت**: بهبود عملکرد برای جریان‌های با داده‌های حجیم (مثل انتقال فایل).  
    - **عیب**: مصرف حافظه بیشتر به ازای هر جریان.  
  - **کاهش مقدار** (مثلاً `32768`):  
    - **مزیت**: مناسب برای جریان‌های کوچک (مثل چت) و کاهش مصرف حافظه.  
    - **عیب**: احتمال کاهش کارایی در انتقال داده‌های بزرگ.

---

### **جمع‌بندی تنظیمات بهینه**
- **برای شبکه‌های با تاخیر بالا (مثل اینترنت ماهواره‌ای)**:  
  افزایش `mux_recievebuffer` و `mux_framesize` می‌تواند مفید باشد.  
- **برای دستگاه‌های با حافظه محدود**:  
  کاهش `mux_streambuffer` و `mux_recievebuffer` توصیه می‌شود.  
- **برای ترافیک حجیم (مثل انتقال فایل)**:  
  افزایش `mux_framesize` و `mux_streambuffer` مناسب است.  

تغییر این مقادیر باید بر اساس تست عملی و شرایط شبکه انجام شود تا بهینه‌ترین عملکرد حاصل گردد.
